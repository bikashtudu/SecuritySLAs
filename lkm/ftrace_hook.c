...
int fh_install_hooks(struct ftrace_hook *hooks, size_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (err)
		{
			pr_debug("error hooking %ld syscall\n",i);
			goto error;
			}
		pr_debug("successful hooking %ld syscall\n",i);
	}
...
static asmlinkage long (*real_sys_openat)(int dfd, 
		const char __user *filename, 
		int flags,
		umode_t mode);

static asmlinkage long fh_sys_openat(int dfd, 
		const char __user *filename, 
		int flags,
		umode_t mode)
{
...
static asmlinkage long fh_sys_read(unsigned int fd, 
	char __user *buf, 
	size_t count)
{
	long ret;
...
static asmlinkage long (*real_sys_write)(unsigned int fd, 
	const char __user *buf,
	size_t count);

static asmlinkage long fh_sys_write(unsigned int fd, 
	const char __user *buf,
	size_t count)
{
	long ret;
